...
void MainWindow::CameraConnectSlot()
{
    ...
    if(this->ui->CameraConnectButton->text()==tr("连接摄像头"))
    {
        if(this->ui->CameraAddress->text()==tr("输入摄像头编号或视频流地址"))
        {
            if(QMessageBox::warning(this,tr("将使用默认摄像头"),tr("未指定输入设备,将使用默认设备"),QMessageBox::Yes|QMessageBox::No)==QMessageBox::Yes)
            {
        ...
        this->ui->CameraConnectButton->setText(tr("正在连接"));
        this->ui->CameraConnectButton->setEnabled(false);
    }
    else if(this->ui->CameraConnectButton->text()==tr("断开"))
    {
        this->Camera->StopCamera();
        this->ui->CameraConnectButton->setText(tr("连接摄像头"));
        this->ui->CameraConnectButton->setText(tr("正在断开"));
        this->ui->CameraConnectButton->setEnabled(false);
    }
}
...
void MainWindow::CameraErrorSlot()
{
    this->ui->CameraConnectButton->setText(tr("连接摄像头"));
    this->ui->CameraConnectButton->setEnabled(true);
    QMessageBox::critical(this,tr("摄像头打开失败"),tr("请检查摄像头连接或视频流地址是否正确"));
}

//摄像头采集信号
void MainWindow::CameraRecordSlot()
{
    if(this->ui->RecordButton->text()==tr("录制视频"))
    {
        this->Camera->StartRecording();
        this->ui->RecordButton->setText(tr("停止录制"));
    }
    else if(this->ui->RecordButton->text()==tr("停止录制"))
    {
